...
# --- Helper for resource path (PyInstaller-safe) ---
...
# --- GUI Setup ---
window = tk.Tk()
window.title("Stereo Camera Recorder")
window.geometry("1000x600")

# Load and display background image
bg_image_path = resource_path("image.png")
bg_img = Image.open(bg_image_path)
bg_img = bg_img.resize((1000, 600))  # Resize to fit window
...
tk.Button(right_frame, text="Start Recording", command=lambda: start_recording(),
          bg="#d63384", fg="white", font=('Helvetica', 12)).pack(pady=10)

# --- Functions ---

def start_recording():
    global recording, goal_frames, save_path, cam0_path, cam1_path, frame_count
    ...
    try:
        ...
        cam0_path = os.path.join(save_path, "cam0")
...
def update_frame():
    ...
    # Save if recording
    if recording and frame_count < goal_frames and ret0 and ret1:
        full_res0 = cv2.resize(frame0, (full_res_width, full_res_height))
        full_res1 = cv2.resize(frame1, (full_res_width, full_res_height))

        filename0 = os.path.join(cam0_path, f"frame_{frame_count:04d}.jpg")
        filename1 = os.path.join(cam1_path, f"frame_{frame_count:04d}.jpg")

        cv2.imwrite(filename0, full_res0)
        cv2.imwrite(filename1, full_res1)
